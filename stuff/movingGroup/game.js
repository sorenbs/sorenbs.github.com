window.onload = function () {
	...
	Crafty.modules({ TiledLevel: 'dev-uncompressed' }, function () {
		...
		Crafty.scene("Game", function () {
			...
			Crafty.modules({ MoveTo: "dev-uncompressed" }, function () {
			...
			/*
			var attackRange = Crafty.e("2D, DOM, radius").attr({ w: 200, h: 200 })
				.origin("bottom left")
				.bind("EnterFrame", function () {
				this.attr({ rotation: Crafty.math.radToDeg(Math.atan2(-200 - this.y + Crafty.mousePos.y, -this.x + Crafty.mousePos.x )) +45 });
			})
				.attr({x: 32, y:32 });
				*/
			//player.attach(attackRange);
		});

		// Components
		Crafty.c("Unit", {
			init: function () {
				this.requires("SpriteAnimation, 2D, DOM, Collision, Solid")
				
				.collision(new Crafty.polygon([5, 5], [25, 25], [5, 25]))
